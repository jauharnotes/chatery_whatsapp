...
class AIService {
    constructor() {
        ...
        this.defaultConfig = {
            ...
            ragEnabled: false,
            ragApiUrl: 'http://localhost:3001/api' // ai-assistant API URL
        };
    ...
    /**
     * Generate AI response using RAG (ai-assistant knowledge base)
     */
    async _generateRAG(config, messageText) {
        const ragUrl = config.ragApiUrl || 'http://localhost:3001/api';
        ...
        if (!response.ok) {
            ...
            throw new Error(error.message || 'RAG API error');
        }

        const data = await response.json();
        
        // ai-assistant returns { answer, usage }
        if (data.answer) {
            return data.answer;
        }
